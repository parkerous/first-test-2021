...
function emailReport() {
  ...
  const subject = `My Voll-E volleyball report — ${p.name} (${p.role})`;
...
function sendViaEmailJS(cfg, p, report) {
  const run = () => {
    ...
    window.emailjs.send(cfg.service, cfg.template, { to_email: p.email, to_name: p.name, report }, cfg.pubkey)
      ...
      .catch(err => { setHint("⚠️ Auto-send failed — opening your email app instead."); const url = `mailto:${encodeURIComponent(p.email).replace(/%40/g, "@")}?subject=${encodeURIComponent("My Voll-E report")}&body=${encodeURIComponent(report)}`; window.location.href = url; });
  };
